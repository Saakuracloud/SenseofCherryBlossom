...
// spotless:off
public class SoCBParallelHelper extends gregtech.api.util.ParallelHelper {
     ...
     */
    private MaxParallelCalculator maxParallelCalculator = SoCBParallelHelper::maxParallelCalculatedByInputs;
//    private MaxParallelCalculator maxParallelCalculator = GTRecipe::maxParallelCalculatedByInputs;
     ...
     */
    private InputConsumer inputConsumer = SoCBParallelHelper::consumeInput;
...
//                  .calculate();
    ...
    public static double maxParallelCalculatedByInputs(GTRecipe recipe, int maxParallel, FluidStack[] aFluidInputs, ItemStack... aInputs) {
        ...
        if (recipe.mInputs.length > 0) {
            Map<CherryBlossomItemID, Long> itemCost = new HashMap<>();
            if (isNBTSensitive) {
                for (ItemStack recipeItemCost : recipe.mInputs) {
                    if (recipeItemCost == null) continue;
                    // for non-consumed input
                    if (recipeItemCost.stackSize == 0) continue;
                    itemCost.merge(CherryBlossomItemID.create(recipeItemCost), (long) recipeItemCost.stackSize, Long::sum);
                }
            } else {
                for (ItemStack recipeItemCost : recipe.mInputs) {
                    if (recipeItemCost == null) continue;
                    // for non-consumed input
                    if (recipeItemCost.stackSize == 0) continue;
                    itemCost.merge(CherryBlossomItemID.createNoNBT(recipeItemCost), (long) recipeItemCost.stackSize, Long::sum);
                }
            }

            nextRecipeItemCost: for (Map.Entry<CherryBlossomItemID, Long> itemID : itemCost.entrySet()) {
